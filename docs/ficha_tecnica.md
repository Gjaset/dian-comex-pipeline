# Ficha Técnica de Fuente de Datos — T1 (DIAN · Importaciones)

Curso **Big Data e Ingeniería de Datos · Universidad Ean**.
Estudiantes: **Germán Cuesta, Jose Alejandro Hernandez, Juan Camilo Pardo**.  

> Todas las cifras de este documento salen de código que se ejecutó sobre
> datos que de verdad se descargaron del sitio público de la DIAN. El notebook
...
> notebook) confirma los números leyéndolos desde `data/mediciones.json`, sin
> necesidad de volver a leer los 5 GB.
...
- **Entidad publicadora:** Dirección de Impuestos y Aduanas Nacionales (DIAN),
  vía su sitio web oficial. Producción estadística certificada por el DANE
  con base en la metodología de la ONU, la Comunidad Andina de Naciones (CAN) y
  los criterios de calidad del DANE.
...
- **Dirección web exacta:**
  https://www.dian.gov.co/dian/cifras/Paginas/Bases-Estadisticas-de-Comercio-Exterior-Importaciones-y-Exportaciones.aspx
- **Endpoint REST descubierto (no documentado oficialmente, pero público):**
  `https://www.dian.gov.co/dian/cifras/_api/web/lists/getbytitle('Bases-estadisticas-importaciones')/items`
  — vista SharePoint abierta sin autenticación, lista todos los archivos mensuales.
- **Períodos elegidos para la medición:** 12 meses de **2023** y 12 meses de
  **2024**. Total: 24 archivos `.zip` ≈ 5.35 GB en `data/raw/`. **Ambos años
  se publican en codificación ALADI** (anterior al cambio M49 de enero-2025);
  por eso se eligieron — para que `g` sea comparable sin artefactos de cambio
  de codificación de país.
...
- **Prohibido** comercializar, adulterar o utilizar las bases con fines
  contrarios a la ley. La responsabilidad por uso indebido recae en el usuario.
- **Obligación de citar:** "Fuente: DIAN — Bases Estadísticas de Comercio
  Exterior" y mencionar la fecha de última actualización usada.

### Formato y mecanismo de publicación
- **Formato:** archivo `.zip` mensual, contiene un único `.xlsx` (≈ 200–270 MB
  comprimido por archivo; ≈ 220–270 MB descomprimido).
- **Mecanismo de descarga:** **NO es API tipo Socrata** (a diferencia de SECOP
  II). Cada mes es un archivo `.zip` completo que se descarga de una sola vez.
  No hay `$limit`/`$offset`, ni timeouts artificiales en consultas de conteo
  como los que ocurrían con datos.gov.co.
- **Organización:** un archivo por mes, 12 archivos por año. Nomenclatura:
  `{NN}_{Importaciones|Exportaciones}_{año}_{Mes}.zip`
  (ej. `01_Importaciones_2023_Enero.zip`).

### Frecuencia declarada y observada
- **Declarada:** publicación mensual con rezago de **45 días posteriores al mes
  de referencia** para importaciones (35 días para exportaciones). Es lo
  normal, no un error de descarga.
- **Observada:** en este proyecto se usaron los 12 meses completos de 2023 y
  2024, todos ya publicados y consolidados. La fecha de última modificación
  reportada por el servidor de la DIAN cae entre noviembre-2024 y noviembre-2025
  (revisión de cifras provisionales) — consistente con la política de cifras
  provisionales del DANE.

### Estabilidad del esquema
**⚠️ No estable — limitación declarada.** Comparando enero-2023 contra
enero-2024 (código en `medicion.ipynb`, celda "Estabilidad del esquema"):
...
- **Adicional — cambio de codificación de país (no observable en estos dos
  años pero documentado):** a partir de enero de 2025, la DIAN adopta el
  estándar internacional **M49** en reemplazo de **ALADI** (Resolución 2386 de
  2023 del DANE). La paramétrica comparativa M49↔ALADI está publicada en la
  *Ficha Técnica de Bases Estadísticas de Comercio Exterior* de la DIAN
...
- En la muestra de las primeras 2 filas, la columna `TIPO_IDENTIFICAC_IMPORTAD`
  aparece vacía (`None`) y `NIT_IMPORTADOR` contiene valores de 9 dígitos
  (NIT, identificadores jurídicos), con `NOMBRE_IMPORTADOR` siendo razones
  sociales ("L V COLOMBIA SAS", no nombres naturales). No se observó columna
  con número de cédula ni nombre de persona natural en esta muestra.
...
- **Resultado: `k = 3.7920`**
  - Memoria deep pandas (5,000 filas): 15.46 MB
  - Tamaño en disco equivalente (5,000 filas, proporcional): 4.08 MB
  - k = 15.46 / 4.08 = 3.79
- Top-5 columnas por consumo de memoria: `DESCRIPCION_MERCANCIA`,
  `RAZON_SOCIAL_DECLARANTE`, `NOMBRE_DECLARANTE`, `DIRECCION_EXPORTADOR`,
  `ADUANA_PRESENTADA` (todas `object`, absorben ~70% de k).
- (código en `medicion.ipynb`, celda "Medición k")

### M · memoria útil disponible
- **M = 14.7682 GB** medidos en el equipo donde se ejecutó el notebook, con el
  kernel y las dependencias ya cargados. Memoria total del equipo: ~23.3 GB.
- Carga del sistema en el momento de la medición: ~37 % usado / 63 %
  disponible (más cargado que en una medición previa del mismo día, donde se
  reportaron 17.37 GB disponibles).
- **Esta cifra es sensible al contexto** — si el equipo tiene navegador, IDE y
  otros procesos abiertos, M baja y el umbral t* se acorta. Es parte normal
  de la definición de M: se mide en el escenario real donde se trabajaría.
...
crecimiento medido directamente sobre los dos períodos reales elegidos — no
una suposición ni una cifra "oficial" de la fuente.
...
- Fórmula (verificada contra el enunciado de la sesión):
  ...
  t* = log(M / (S0 · k)) / log(1 + g)
     = log(14.7682 / (2.8913 · 3.7920)) / log(1 + 0.051354)
     = log(14.7682 / 10.9636) / log(1.051354)
     = log(1.3469) / log(1.051354)
     = 0.2975 / 0.050074
     = 5.944  períodos (años) desde 2024
  ```

- **Resultado: `t* = 5.95 años` desde 2024**
- **S₀·k = 10.96 GB** (memoria que ocuparía cargar un año completo de
  importaciones en pandas con `deep=True`).
- **M = 14.77 GB** — hoy todavía cabe, con un margen de ~3.8 GB, pero el
  dataset crece ~5 % anual y M no crece.

### Interpretación en una frase
Con el volumen actual de 2024 (S₀=2.89 GB, k=3.79), cargar el año completo en
pandas consume ya **10.96 GB de RAM** frente a **14.77 GB disponibles**; al
ritmo de crecimiento medido (+5.14 % anual), el dataset anual dejará de caber
en la memoria del equipo de trabajo dentro de **aproximadamente 6 años**
(~2030). Basta con reducir M (abrir más procesos, tener menos RAM de equipo
o trabajar con dos años a la vez) para acortar este margen de forma
significativa.

### Sensibilidad de t* a M (no omitirlo)
`t*` es sensible a M. Como referencia:
- con **M = 12 GB** (equipo ocupado en otra tarea pesada): t* ≈ **3.6 años**.
- con **M = 14.77 GB** (medición real de hoy): t* ≈ **5.95 años**.
- con **M = 17.4 GB** (medición anterior el mismo día, equipo más libre):
  t* ≈ **7.55 años**.
...
- En qué parte: descubrimiento del endpoint SharePoint `_api` abierto de la
  DIAN (vía inspección del HTML de la página oficial); generación del notebook
  `medicion.ipynb` y del script `src/run_medicion.py` que regenera el notebook
  sin pasar por un kernel Jupyter (para evitar el problema de Python 3.14 vs
  3.11 en el equipo del estudiante); esqueleto del documento y fórmulas
  matemáticas.
- Qué verifiqué contra ejecución real: **todas las cifras** se obtuvieron
  ...
  (23.3 GB totales, 14.77 GB disponibles en el momento de la medición). El
